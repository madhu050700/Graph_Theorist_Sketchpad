    ...
    def EdgeDrawer(background, color):
        ...
        for edge in Edge.all:
            ...
            if edge.vertex1 == edge.vertex2:
                pygame.draw.line(background, color, edge.vertex1.pos,
                                 [edge.vertex2.pos[0] + 50, edge.vertex2.pos[1] + 50], 2)
                pygame.draw.line(background, color, [edge.vertex2.pos[0] + 50, edge.vertex2.pos[1] + 50],
                                 [edge.vertex2.pos[0] + 100, edge.vertex2.pos[1]], 2)
                pygame.draw.line(background, color, [edge.vertex2.pos[0] + 100, edge.vertex2.pos[1]],
                                 [edge.vertex2.pos[0] + 50, edge.vertex2.pos[1] - 50], 2)
                pygame.draw.line(background, color, [edge.vertex2.pos[0] + 50, edge.vertex2.pos[1] - 50],
                                 edge.vertex2.pos, 2)
                if edge.num > 1:
                ...
                if edge.num > 1:
                    pos = [(edge.vertex1.pos[0] + edge.vertex2.pos[0]) / 2,
                           (edge.vertex1.pos[1] + edge.vertex2.pos[1]) / 2]
                    text = font.render(f'{edge.num}', True, color)
                    background.blit(text, pos)
    ...
    def are_connected(StartVertex, EndVertex):
        ...
        while not Vertex.are_directly_connected(CurrentVertex, EndVertex):
            ...
            for index in connections:
                if (Vertex.get(int(index)) not in CurrentPathOfVertices) and (
                        Vertex.get(int(index)) not in CompletedVertices):
                ...
                while not done:
                    ...
                    for edge in CurrentVertex.connections():
                        if edge == str(CurrentVertex.index):
                            return False
                    if color % 2 == 0:
                        if CurrentVertex not in white:
                            white.append(CurrentVertex)
...
class Component:
    ...
    def ComponentJoiner(self, Edges):
        ...
        for index in connections:
            if Vertex.get(
                    int(index)) not in self.vertices:  # vertex is in anothher component, so we need to find which one it is in.
                ...
                for comp in LoopComponents:
                    ...
                    if Vertex.get(
                            int(index)) in comp.vertices:  # then our self and this comp are connected, so we need to join them
...
ZEROINTENSITY = 0
MAXINTENSITY = 255
COLOR = [(255,0,0),(0,0,255),(255,0,255),(0,255,0),(130,120,165),(200,210,230)]
...
showdegree = False
bipartite = False
button_connected = pygame.Rect(0, 0, 140, 30)
button_bipartie = pygame.Rect(0, 30, 140, 30)
button_matrix = pygame.Rect(0, 60, 140, 30)
button_hamiltonian = pygame.Rect(0, 90, 140, 30)
button_vertex = pygame.Rect(WINDOW_SIZE[0]-400, 120, 200, 40)
button_edge = pygame.Rect(WINDOW_SIZE[0]-400, 150, 200, 40)
button_delete = pygame.Rect(WINDOW_SIZE[0]-400, 180, 200, 40)
button_degree= pygame.Rect(WINDOW_SIZE[0]-400, 210, 200, 40)
button_move_vertex= pygame.Rect(WINDOW_SIZE[0]-400, 240, 200, 40)
font = pygame.font.SysFont("comicsans", 25)
...
while is_running:
    ...
    for event in pygame.event.get():
        ...
        if (event.type == pygame.MOUSEBUTTONDOWN) and (KEY == 'e'):
            for vertex in Vertex.all:
                if (((pos[0] >= vertex.pos[0] - RADIUS) and (pos[0] <= vertex.pos[0] + RADIUS)) and (
                        (pos[1] >= vertex.pos[1] - RADIUS) and (pos[1] <= vertex.pos[1] + RADIUS))):
        ...
        if (event.type == pygame.MOUSEBUTTONDOWN) and (KEY == 'd'):  # delete any vertex we touch
            ...
            for edge in Edge.all:
                ...
                if (distance_point_line(pos, [edge.vertex1.pos[0], edge.vertex1.pos[1]],
                                        [edge.vertex2.pos[0], edge.vertex2.pos[1]]) < 8):
    ...
    ButtonDrawer(background, [128, 128, 0], button_vertex, "Vertex Creation", (255, 255, 255), [WINDOW_SIZE[0]-400, 120])

    ButtonDrawer(background, [128, 128, 0], button_edge, "Edge Creation", (255, 255, 255), [WINDOW_SIZE[0] - 400, 150])

    ButtonDrawer(background, [128, 128, 0], button_delete, "Deletion", (255, 255, 255), [WINDOW_SIZE[0] - 400, 180])

    ButtonDrawer(background, [128, 128, 0], button_degree, " Degree", (255, 255, 255), [WINDOW_SIZE[0] - 400, 210])

    ButtonDrawer(background, [128, 128, 0], button_move_vertex, " Move Vertex", (255, 255, 255), [WINDOW_SIZE[0] - 400, 240])

    if KEY == 'v':
        textv = font.render(f'Vertex Creation Mode', True, (255, 255, 255))
        background.blit(textv, [WINDOW_SIZE[0] - 600, 30])

    elif KEY == 'e':
        texte = font.render(f'Edge Creation Mode', True, (255, 255, 255))
        background.blit(texte, [WINDOW_SIZE[0] - 600, 30])
    elif KEY == 'd':
        textd = font.render(f'Deletion Mode', True, (255, 255, 255))
        background.blit(textd, [WINDOW_SIZE[0] - 600, 30])
    elif KEY == 'm':
        textm = font.render(f'Vertex moving mode', True, (255, 255, 255))
        background.blit(textm, [WINDOW_SIZE[0] - 600, 30])
    ...
    if len(Vertex.all)  >= 1:
        matrix = myfont.render(f'Matrix = {AdjacencyMatrix()}', True, (255, 160, 122))
        background.blit(matrix, [WINDOW_SIZE[0] - 1200, 0])

        bipar = font.render(f'Bipartite = {Vertex.bipartie()}', True, (255, 160, 122))
        background.blit(bipar, [WINDOW_SIZE[0] - 1200, 30])

        conn = font.render(f'Connected= {Vertex.connected(Vertex.all)}', True, (255, 160, 122))
        background.blit(conn, [WINDOW_SIZE[0] - 1200, 60])

        hamil = font.render(f'Hamiltonian= {Hamiltonian()}', True, (255, 160, 122))
        background.blit(hamil, [WINDOW_SIZE[0] - 1200, 90])



    Edge.EdgeDrawer(background, COLOR[2])
    Vertex.VertexDrawer(background, COLOR[2], RADIUS, bipartite)
    if showdegree == True:
